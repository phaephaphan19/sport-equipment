...
function loadBorrowTable(page = 1) {
    ...
    fetch(`/sport_system/php/borrow.php?action=list_borrowed&page=${page}`)
        .then(res => res.json())
        .then(result => {
...
function loadHistoryTable(page = 1) {
    ...
    fetch(`/sport_system/php/borrow.php?action=list_history&page=${page}`)
        .then(res => res.json())
        .then(result => {
...
function returnEquipment(borrow_id) {
    ...
    fetch("/sport_system/php/borrow.php?action=return_equipment", {
        method: "POST",
        body: formData
    })
        .then(res => res.json())
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (borrowForm) {
        borrowForm.addEventListener("submit", function (e) {
            ...
            fetch("/sport_system/php/borrow.php?action=borrow_equipment", {
                method: "POST",
                body: formData
            })
                .then(res => res.json())
    ...
    if (resetHistoryBtn) {
        resetHistoryBtn.addEventListener("click", function () {
            ...
            if (!confirm("⚠️ คุณต้องการลบประวัติการยืม–คืนทั้งหมดจริงหรือไม่?")) return;

            fetch("/sport_system/php/borrow.php?action=reset_history", {
                method: "POST"
            })
                .then(res => res.json())
                .then(result => {
